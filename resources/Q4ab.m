function [cptHypo,cptHypo50] = Q4ab(Charges,Smoker,Age)
...
%% Partie A:  calcule des frais hospitalier moyen des fumeur et non fumeur 
i=1;
cptHypo = 0;
while(i<=100)
    sumSmoker = 0;
...
%% Partie b personne de plus de 50 ans seulement
...
while(i<=100)
    ...
    j =1;
    while(j<=50)
        if(echantillon(multisample(i,j),2) == 1 && echantillon(multisample(i,j),3) > 50)
            sumSmoker50 = sumSmoker50 + echantillon(multisample(i,j),1);
            cptSmoker50 = cptSmoker50 + 1;
        elseif(echantillon(multisample(i,j),2) == 0 && echantillon(multisample(i,j),3) > 50)
            sumnonSmoker50 = sumnonSmoker50 + echantillon(multisample(i,j),1);
            cptnonSmoker50 = cptnonSmoker50 + 1;
        end
        j = j+1;
    end
    moySmoker50 = sumSmoker50/cptSmoker50;
    moynonSmoker50 = sumnonSmoker50/cptnonSmoker50;
    if(moySmoker50 > moynonSmoker50)
        cptHypo50 = cptHypo50 + 1;
    end
    i = i+1;
end
